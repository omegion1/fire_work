...
async function main() {
    try {
        ...
        const outputStream = fs.createWriteStream('wallets.txt', { flags: 'a' });
        let successCount = 0;
        const batchSize = 10;

        for (let i = 0; i < numWallets; i += batchSize) {
            console.log(`\n🚀 Running batch: Wallets ${i + 1} to ${Math.min(i + batchSize, numWallets)}`);
            
            const batchPromises = [];
            for (let j = 0; j < batchSize && (i + j) < numWallets; j++) {
                const wallet = await generateWallet();
                const proxy = proxies.length > 0 ? proxies[(i + j) % proxies.length] : null;
                batchPromises.push(processWalletAndTasks(wallet, "VPZ7DA", outputStream, i + j, numWallets, proxy));
            }
        ...
        console.log('📝 Check wallets.txt for wallet information');
